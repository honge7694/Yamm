...
const ImageItem = ({ image, idx, hit, content, tag, create_date, user_info, reaction}) => {
    ...
    const routeFuntion = () => {
        Router.push({
            ...
            query: { "image": image, "hit" : hit, "content" : content, "idx" : idx, "tag" : tag, "create_date" : create_date, "user_info1" : user_info ,
                   "nickname" : user_info["nickname"], "profile_img" : user_info["profile_img"], "reaction" : reaction  } 
        });
    }
    return (
        <>
